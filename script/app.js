...
function renderTask(tasks){
    ...
    tasks.forEach((taskInfo, index) => {
        ...
        $taskElement.innerHTML = `
            <h3 contenteditable="${taskInfo.isBeingEdited}"  style="${taskInfo.isCompleted ? 'background: green; color: #fff' : ''}">${taskInfo.taskName}</h3>
            <div data-item-id="${index}">
                <button class="complete-btn">Complete</button>
                <button class="edit-btn">${taskInfo.isBeingEdited ? "Save" : "Edit"}</button>
                <button>${String(taskInfo.hour).padStart(2, "0")} : ${String(taskInfo.minute).padStart(2, "0")}
                    <small class="task-date">${String(taskInfo.date).padStart(2, "0")}/${String(taskInfo.month).padStart(2, "0")}/${taskInfo.year}</small>
                </button>
...
$taskCollection.addEventListener("click", (e) => {
    ...
    if(e.target.className === "edit-btn"){
        ...
        renderTask(ALL_TASKS);
        ALL_TASKS[id].taskName = e.target.parentElement.previousElementSibling.textContent;
        localStorage.setItem("alltasks", JSON.stringify(ALL_TASKS))
        renderTask(ALL_TASKS);
    }
})